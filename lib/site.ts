...
export const seoTools: SeoTool[] = [
  ...
  {
    slug: 'freeze-frame-video',
    toolId: 'freeze',
    name: 'Freeze Frame Video Editor',
    title: 'Freeze a Video Frame Online Free — Hold Frame Effect',
    description: 'Freeze an exact video frame for a custom duration, then continue playback. Local synchronized rendering with no upload or watermark.',
    answer: 'EditLocal creates a freeze-frame effect at the exact timestamp you choose. It holds the selected frame for a custom duration, inserts matching silence to keep later footage synchronized, and renders the result locally.',
    keywords: ['freeze video online', 'freeze frame video editor', 'hold frame effect', 'pause video frame no watermark'],
    features: ['Exact decimal-second timestamp', '0.5 to 10 second quick duration control', 'Up to 30 seconds with exact entry', 'Synchronized audio timeline', 'Local high-quality rendering'],
    steps: ['Choose a video.', 'Select the exact freeze timestamp.', 'Set how long the frame should remain still.', 'Render locally and download the result.'],
    limitations: 'The freeze frame extends the video duration and requires re-encoding. If a source has no audio track, EditLocal automatically produces a silent freeze-frame result.',
    faq: [
      { question: 'Does the video continue after the frozen frame?', answer: 'Yes. The chosen frame is held for the selected duration, then the remaining footage continues.' },
      { question: 'Will audio stay synchronized?', answer: 'Yes. Matching silence is inserted during the freeze so later video and audio remain aligned.' },
      { question: 'Is the freeze-frame video uploaded?', answer: 'No. The complete operation runs locally in your browser.' },
    ],
  },
  {
    slug: 'reverse-video',
    toolId: 'reverse',
    name: 'Reverse Video Editor',
    title: 'Reverse Video Online Free — Play Video Backwards',
    description: 'Reverse a video locally with optional reversed audio. Free no-watermark playback reversal with no account or server upload.',
    answer: 'EditLocal reverses every decoded video frame directly in your browser and can reverse the audio track as well. Turn audio reversal off for a silent backwards video, then export without uploading the source.',
    keywords: ['reverse video online free', 'play video backwards', 'reverse MP4', 'reverse video no watermark'],
    features: ['Full backwards playback', 'Optional reversed audio', 'Automatic silent fallback', 'High-quality MP4 export', 'Private local processing'],
    steps: ['Choose a video.', 'Choose whether to reverse the audio.', 'Select the desired quality.', 'Reverse locally and download.'],
    limitations: 'Reversing requires decoded frames to be buffered in device memory. Short clips process most smoothly; trim long or high-resolution videos before reversing them on memory-limited devices.',
    faq: [
      { question: 'Can I reverse the sound too?', answer: 'Yes. Keep Reverse audio too enabled to reverse picture and sound together.' },
      { question: 'Can I make a silent reversed video?', answer: 'Yes. Turn off Reverse audio too before processing.' },
      { question: 'Why do long reverse jobs use more memory?', answer: 'Frames must be buffered before they can be emitted in the opposite order. This is inherent to exact video reversal.' },
    ],
  },
  {
    slug: 'video-to-gif',
  ...
  {
    slug: 'remove-green-screen-from-video',
    toolId: 'greenscreen',
    name: 'Green Screen Remover',
    title: 'Remove Green Screen from Video Free — Chroma Key Online',
    description: 'Remove a green background locally with adjustable chroma-key tolerance and edge feathering. Export transparent WebM or a solid-background MP4.',
    answer: 'EditLocal removes green screen backgrounds directly in your browser. Select the key color, refine tolerance and edge feathering, then export transparent video or replace the background with a solid color without uploading the source.',
    keywords: ['remove green screen from video', 'chroma key online free', 'transparent video maker', 'green screen remover no upload'],
    features: ['Custom key-color picker', 'Adjustable key tolerance', 'Soft edge feathering', 'Transparent WebM and solid-color MP4', 'Local FFmpeg processing'],
    steps: ['Choose a green-screen video.', 'Pick the key color and tune tolerance.', 'Adjust edge feathering.', 'Choose transparent or solid-color output and render locally.'],
    limitations: 'Chroma key works best with even lighting and clear separation between the subject and screen. Green reflections, motion blur, shadows, and similarly colored clothing can require careful tolerance adjustment.',
    faq: [
      { question: 'Can I make the video background transparent?', answer: 'Yes. Choose Transparent WebM to preserve an alpha channel in a compatible video format.' },
      { question: 'Can I key a color other than green?', answer: 'Yes. Use the key-color picker to select blue or another evenly lit background color.' },
      { question: 'Is the video uploaded?', answer: 'No. Chroma-key rendering happens in your browser on the current device.' },
    ],
  },
  {
    slug: 'add-voiceover-to-video',
    toolId: 'voiceover',
    name: 'Video Voiceover Editor',
    title: 'Add Voiceover to Video Free — Record or Upload Audio',
    description: 'Record a narration or add an audio file to video locally. Replace the original sound or mix both tracks with independent volume controls.',
    answer: 'EditLocal lets you record a voiceover from your microphone or choose an existing audio track, then add it to a video locally. Replace the original soundtrack or mix narration and source audio at separate volume levels.',
    keywords: ['add voiceover to video', 'record narration online', 'voice over video free', 'mix audio with video no upload'],
    features: ['Browser microphone recording', 'Audio-file upload', 'Replace or mix modes', 'Independent volume controls', 'Private local rendering'],
    steps: ['Choose a video.', 'Record narration or choose an audio file.', 'Replace the original audio or mix the tracks.', 'Set volume levels and render the result.'],
    limitations: 'Microphone recording requires browser permission and a secure HTTPS page. Mix mode requires the source video to contain an audio track; use Replace audio when it does not.',
    faq: [
      { question: 'Can I record a voiceover inside EditLocal?', answer: 'Yes. Select Record here, allow microphone access, speak, and stop when finished.' },
      { question: 'Can I keep background music?', answer: 'Yes. Choose Mix together and independently adjust the original and narration volumes.' },
      { question: 'Does EditLocal upload my recording?', answer: 'No. The recording remains in the current browser tab and is processed locally.' },
    ],
  },
  {
    ...
    title: 'Add Text and Captions to Video Free — No Watermark',
    description: 'Add multiple timed text captions to video locally. Control timing, position, text size, color, and background readability with no watermark.',
    answer: 'EditLocal burns multiple timed captions directly into a video in your browser. Each caption has start and end times, while shared style controls adjust placement, text color, size, and background opacity.',
    keywords: ['add captions to video', 'add text to video free', 'video caption editor no watermark', 'burn subtitles into video'],
    features: ['Multiple timed caption cues', 'Top, center, and bottom positions', 'Text size and color controls', 'Readable background opacity', 'Local no-watermark export'],
    steps: ['Choose a video.', 'Enter caption text and start/end times.', 'Choose position, text size, color, and background.', 'Preview the style and render locally.'],
    limitations: 'This version creates hardcoded captions that remain visible in the exported pixels. It does not automatically transcribe speech or create a selectable subtitle track.',
    faq: [
      { question: 'Can I add more than one caption?', answer: 'Yes. Add up to eight timed caption cues in one export.' },
      { question: 'Can viewers turn these captions off?', answer: 'No. They are burned into the video so they display consistently everywhere.' },
      { question: 'Does the exported video have a watermark?', answer: 'No. EditLocal does not add branding to the result.' },
    ],
  },
  {
    slug: 'compress-image',
