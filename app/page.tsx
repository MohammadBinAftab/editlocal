  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <header className="sticky top-0 z-30 flex h-16 items-center border-b border-border/80 bg-background/90 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
        <button className="mr-3 rounded-xl p-2 text-muted-foreground hover:bg-muted lg:hidden" aria-label="Open tools"><Menu className="size-5" /></button>
      ...
      <div className="border-b border-border bg-card px-4 py-2 lg:hidden">
        <div className="flex gap-2 overflow-x-auto pb-1">
          {tools.slice(0, 10).map((item) => (
            <button key={item.id} onClick={() => chooseTool(item.id)} className={`shrink-0 rounded-full px-3 py-1.5 text-xs font-medium ${activeTool === item.id ? 'bg-primary text-primary-foreground' : 'bg-muted text-muted-foreground'}`}>{item.label}</button>
          ))}
        </div>
      </div>
      ...
      <div className="mx-auto grid max-w-[1540px] grid-cols-1 lg:grid-cols-[244px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 px-4 py-7 sm:px-7 lg:px-10 lg:py-9">
          ...
          <div className="grid overflow-hidden rounded-[22px] border border-border bg-card shadow-[0_24px_80px_-52px_rgba(28,25,55,.32)] xl:grid-cols-[minmax(0,1.35fr)_400px]">
            <div
              className="relative flex min-h-[500px] items-center justify-center overflow-hidden bg-[#121119] p-7 sm:min-h-[640px]"
            ...
            >
              ...
              <div className="absolute bottom-5 left-5 right-5 z-10 flex items-center justify-between text-xs text-white/55">
                <span className="max-w-[70%] truncate">{files.length ? `${files.length > 1 ? `${files.length} files` : files[0].name} · ${formatSize(files.reduce((sum, file) => sum + file.size, 0))}` : 'Your file never leaves this device'}</span>
                ...
                {activeTool === 'reframe' && <fieldset>
                  ...
                  <div className="space-y-2">
                    {['Smart reframe', 'Background fill', 'Fit entire video'].map((value) => (
                      <button key={value} onClick={() => setFraming(value)} className={`flex w-full items-center rounded-xl border p-3 text-left transition ${framing === value ? 'border-violet-300 bg-violet-50' : 'border-border hover:bg-muted/60'}`}>
                        ...
                        <span><span className="block text-sm font-semibold">{value}</span><span className="mt-0.5 block text-[11px] text-muted-foreground">{value === 'Smart reframe' ? 'Tracks motion and visual detail' : value === 'Background fill' ? 'Keep every source pixel' : 'Letterbox with no crop'}</span></span>
