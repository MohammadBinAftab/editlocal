...
export type MediaAppProps = {
  initialTool?: ToolId;
  focused?: boolean;
};

export function MediaApp({ initialTool = 'reframe', focused = false }: MediaAppProps) {
  ...
  const [format, setFormat] = useState(() => tools.find((item) => item.id === initialTool)?.category === 'Photo' ? 'JPG' : 'MP4');
  ...
  useEffect(() => {
    ...
    if (matchedTool) router.replace(`/tools/${toolSlugs[matchedTool.id]}`);
  }, [router]);
  ...
  const chooseTool = (id: ToolId) => {
    ...
    router.push(`/tools/${toolSlugs[id]}`);
  };
    ...
    <main className="min-h-screen bg-background text-foreground">
      ...
      <div className="mx-auto grid max-w-[1540px] grid-cols-1 lg:grid-cols-[244px_minmax(0,1fr)]">
        ...
        <section className="min-w-0 px-3 py-6 sm:px-7 sm:py-7 lg:px-10 lg:py-9">
          <section className={`${focused ? 'hidden' : ''} rounded-[26px] border border-violet-100 bg-[linear-gradient(135deg,#f5f1ff_0%,#fff_55%,#eefcf7_100%)] p-5 sm:p-8`}>
          ...
          <section className={`${focused ? 'hidden' : ''} mt-8`} aria-labelledby="all-dashboard-tools">
            ...
            <div className="grid gap-3 sm:grid-cols-2 2xl:grid-cols-3">
              {tools.map(({ id, label, description, icon: Icon, category }) => (
                <Link key={id} href={`/tools/${toolSlugs[id]}`} className="group rounded-2xl border border-border bg-card p-4 transition hover:-translate-y-0.5 hover:border-violet-200 hover:shadow-lg focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-violet-400">
                  ...
                  <span className="mt-4 inline-flex min-h-10 items-center gap-1.5 rounded-xl bg-primary px-3 text-xs font-semibold text-primary-foreground">Open editor <ArrowRight className="size-3.5 transition group-hover:translate-x-0.5" /></span>
                </Link>
              ))}
            </div>
          </section>

          <div id="editor" className={`${focused ? 'flex' : 'hidden'} mb-6 flex-wrap items-end justify-between gap-4`}>
            <div>
          ...
          <div className={`${focused ? 'grid' : 'hidden'} overflow-hidden rounded-2xl border border-border bg-card shadow-[0_24px_80px_-52px_rgba(28,25,55,.32)] sm:rounded-[22px] xl:grid-cols-[minmax(0,1.35fr)_400px]`}>
            <div
